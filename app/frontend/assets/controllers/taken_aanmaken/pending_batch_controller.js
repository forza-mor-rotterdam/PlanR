...
export default class extends Controller {
  ...
  ensurePlaceholders() {
    ...
    const insertionPoint = this.placeholderContainer.querySelector('.container__aanmaken')
    ...
    this.taken.forEach((taak) => {
      ...
      summary.innerHTML = `
        <div class="description">
          <div class="wrapper">
            <p>${this.escapeHtml(taak.titel || 'Taak')}</p>
          </div>
          <small></small>
        </div>
        ...
        <div><span class="help-text"></span></div>
        <div><span class="tag tag--qualification">Wordt aangemaakt</span></div>
        ${
          hasOptionsColumn
            ? `<div>
      ...
      if (insertionPoint) {
        this.placeholderContainer.insertBefore(row, insertionPoint)
      } else {
        this.placeholderContainer.appendChild(row)
      }
    })
